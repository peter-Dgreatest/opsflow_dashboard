  ...
  return (
    <aside className={`${collapsed ? 'w-14' : 'w-52'} shrink-0 bg-card border-r border-border flex flex-col transition-all duration-200 overflow-hidden z-10`}>
      ...
      <div className="flex items-center gap-2.5 px-3.5 py-4 border-b border-border">
        ...
        {!collapsed && <span className="font-semibold text-sm tracking-tight text-text"><span className="text-lime-500">Ops</span>Flow</span>}
      </div>
        ...
        {SECTIONS.map(sec => {
          ...
          return (
            <div key={sec.key} className="mb-1">
              {!collapsed && (
                <p className="text-[9px] font-bold uppercase tracking-widest text-muted px-2.5 py-1.5 mt-1 opacity-50">{sec.label}</p>
              )}
              {items.map(item => {
                ...
                return (
                  <button key={item.key}
                    ...
                    className={`w-full flex items-center gap-2.5 px-2 py-2 rounded-lg text-left transition-all duration-100 cursor-pointer mb-0.5
                      ${isActive ? 'bg-black/[0.06] dark:bg-white/[0.07]' : 'hover:bg-black/[0.04] dark:hover:bg-white/[0.04]'}`}>
      ...
      <div className="border-t border-border p-2 space-y-1">
        ...
        <button
          ...
          className={`w-full flex items-center gap-2 px-2 py-2 rounded-lg text-muted hover:text-text transition-colors cursor-pointer
...
export function Topbar({ title, subtitle, actions }) {
  ...
  return (
    <header className="h-14 shrink-0 bg-card border-b border-border flex items-center gap-4 px-5">
      ...
      {/* Theme toggle in topbar too */}
      <button
        onClick={toggle}
        title={theme === 'dark' ? 'Light mode' : 'Dark mode'}
        className="w-7 h-7 rounded-lg bg-input border border-border flex items-center justify-center text-muted hover:text-text hover:bg-black/5 dark:hover:bg-white/10 transition-colors cursor-pointer">
        {theme === 'dark' ? <Sun size={13} /> : <Moon size={13} />}
      </button>
      <button className="w-7 h-7 rounded-lg bg-input border border-border flex items-center justify-center text-muted hover:text-text hover:bg-black/5 dark:hover:bg-white/10 transition-colors cursor-pointer">
        <Bell size={13} />
      </button>
    </header>
  );
